/*
Vuex module to retrieve the conformance classes json of a pygeoapi instance
*/
...
const getters = {
  ...
  // from json
  conformsTo: (state, getters) => {
    if (state.jsonLoaded) {
      return getters.json.conformsTo
    } else {
      return []
    }
  }
...
const actions = {
  getJson({commit, state}) {
    ...
    axios.get(PYGEOAPI_HOST + '/conformance?f=json')
      .then((res) => {
